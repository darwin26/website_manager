...
class rex_website_manager {
	...
	public static function createWebsite($websiteId) {
		...
		if ($REX['WEBSITE_MANAGER_SETTINGS']['identical_clangs']) {
			// identical
			$sql->setQuery('CREATE VIEW ' . $tablePrefix . 'clang AS SELECT * FROM rex_clang');
		} else {
			// different
		...
		if ($REX['WEBSITE_MANAGER_SETTINGS']['identical_media']) {
			// identical
			$sql->setQuery('CREATE VIEW ' . $tablePrefix . 'file AS SELECT * FROM rex_file');
			$sql->setQuery('CREATE VIEW ' . $tablePrefix . 'file_category AS SELECT * FROM rex_file_category');
		} else {
			// different
		...
		if ($REX['WEBSITE_MANAGER_SETTINGS']['identical_image_types']) {
			// identical
			$sql->setQuery('CREATE VIEW ' . $tablePrefix . '679_types AS SELECT * FROM rex_679_types');
			$sql->setQuery('CREATE VIEW ' . $tablePrefix . '679_type_effects AS SELECT * FROM rex_679_type_effects');
		} else {
			// different (addon will be reinstalled)
	...
	public static function destroyWebsite($websiteId) {
		...
		// clangs
		if ($REX['WEBSITE_MANAGER_SETTINGS']['identical_clangs']) {
			$sql->setQuery('DROP VIEW ' . $tablePrefix . 'clang');
		} 

		// media
		if ($REX['WEBSITE_MANAGER_SETTINGS']['identical_media']) {
			$sql->setQuery('DROP VIEW ' . $tablePrefix . 'file');
			$sql->setQuery('DROP VIEW ' . $tablePrefix . 'file_category');
		}
		...
		if ($REX['WEBSITE_MANAGER_SETTINGS']['identical_meta_infos']) {
			$sql->setQuery('DROP VIEW ' . $tablePrefix . '62_type');
			$sql->setQuery('DROP VIEW ' . $tablePrefix . '62_params');
		}

		if ($REX['WEBSITE_MANAGER_SETTINGS']['identical_image_types']) {
			$sql->setQuery('DROP VIEW ' . $tablePrefix . '679_types');
			$sql->setQuery('DROP VIEW ' . $tablePrefix . '679_type_effects');
		}
		...
		if (!$REX['WEBSITE_MANAGER_SETTINGS']['identical_media']) {
			rex_website_manager_utils::rrmdir($htdocsPath . $filesDir);
		}
	}
}
